    ...
    public class MongoCottageRepository : MongoBuildingRepository
    {
        ...
        public override ErrorCode CreateFloor(int buildingId, Floor data)
        {
            try
            {
                ...
                if (cottage.Floors.Any(r => r.Number == data.Number))
                    return ErrorCode.FloorExist;

                cottage.Floors.Add(data);
        ...
        public override ErrorCode DeleteFloor(int buildingId, int floorId)
        {
            ...
            {
                Cottage cottage = this.repository.Single(r => r.BuildingId == buildingId);
                Floor floor = cottage.Floors.SingleOrDefault(r => r.FloorId == floorId);
                if (floor == null)
                    return ErrorCode.ObjectNotFound;

                cottage.Floors.Remove(floor);
