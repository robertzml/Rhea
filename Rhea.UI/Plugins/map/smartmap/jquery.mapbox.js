...
(function ($) {// jQuery.noConflict compliant
    $.fn.mapbox = function (o, callback) {
        ...
        var method = {//public methods
            ...
            setSelect: function () {
                _setStatus.call(this, "select");
            },
            setClick: function () {
                _setStatus.call(this, "click");
            },
            setPin: function () {
                _setStatus.call(this, "pin");
            },
            setSearch: function () {
                _setStatus.call(this, "search");
            }
        }
        ...
        return this.each(function () {
            if (typeof command == "string") {//execute public methods if called
                ...
                this.visible = o.defaultLayer, this.defaultLayer = o.defaultLayer, this.layerSplit = o.layerSplit; //magic
                ...
                */
                ...
                $(this).bind(FUI.env.ua.clickEventDown, function (event) {
                    ...
                    if (FUI.env.ua.ontouch) {
                        event.preventDefault();
                        e = event.originalEvent.touches[0] || event.originalEvent.changedTouches[0];
                    }

                    o.callBefore(layer, e.clientX + x, e.clientY + y, viewport);
                ...
                $(this).bind(FUI.env.ua.clickEventUp, function (event) {
                    //
                    ...
                    //
                    if (this.status == "select") {
                        var top = parseInt($(this).find(".map-layer-select").css('top'));
                        var left = parseInt($(this).find(".map-layer-select").css('left'));
                        o.afterSelect(layer, left + x, top + y, e.clientX + x, e.clientY + y);
                        $(this).find(".map-layer-select").remove();
                    }
                    else {
                        mapmove = false;
                        if (weveMoved) {
                            ...
                            o.callAfter(layer, e.clientX + x, e.clientY + y, viewport);
                        }
                    }
                    weveMoved = false;
                    return false;
                ...
                if (o.mousewheel && typeof $.fn.mousewheel != "undefined") {
                    $(viewport).mousewheel(function (e, distance) {
                        ...
                        //
                        var level = distance > 0 ? 1 : -1;
                        var step = 90 / (this.layerSplit - this.defaultLayer - 1) / 5;
                        var y = (parseInt($('.map-drop').css('top')) - step * level);
                        if (y < 0) {
                            $('.map-drop').css({ "top": 0 });
                            return false;
                        }
                        ...
                        $('.map-drop').css({ "top": y });
                        //
                        if (_zoom.call(this, level) && o.zoomToCursor/* && distance > 0*/) {
                ...
                if (o.doubleClickZoom || o.doubleClickZoomOut || o.doubleClickMove) {
                    $(viewport).dblclick(function (e) {
                        ...
                        //
                        var step = 90 / (this.layerSplit - this.defaultLayer - 1) / 5;
                        var y = (parseInt($('.map-drop').css('top')) - step * distance);
                        if (y < 0) y = 0;
                        if (y > 90) y = 90;
                        $('.map-drop').css({ "top": y });
                        //convert percentage to pixels on new layer
                ...
                if (o.clickZoom || o.clickZoomOut || o.clickMove) {
                    $(viewport).bind(FUI.env.ua.clickEventDown, function (e) {
                ...
                //
                $(".map-control a").bind(FUI.env.ua.clickEventDown, { handle: this }, function (e) {
                    if (this.className == "zoom") {
                        var step = 90 / (e.data.handle.layerSplit - e.data.handle.defaultLayer - 1) / 5;
                        var y = (parseInt($('.map-drop').css('top')) - step);

                        if (y < 0)
                            y = 0;
                        //else
                        _zoom.call(e.data.handle, 1);
                        //
                        $('.map-drop').css({ "top": y });
                    }
                        ...
                        _move.call(e.data.handle, -30, 0);
                    }
                    else if (this.className == "right") {
                        _move.call(e.data.handle, 30, 0);
                    }
                    else if (this.className == "up") {
                        _move.call(e.data.handle, 0, -30);
                    }
                    else if (this.className == "down") {
                        _move.call(e.data.handle, 0, 30);
                    }
                    else if (this.className == "center") {
                        var coords = {
                            x: $(e.data.handle).find(".current-map-layer").width() / 2,
                            y: $(e.data.handle).find(".current-map-layer").height() / 2
                        }
                        var node = $(e.data.handle).find(".current-map-layer");
                        var newX = coords.x - ($(e.data.handle).width() / 2), newY = coords.y - ($(e.data.handle).height() / 2);
                        _position.call(e.data.handle, newX, newY, node[0]);
                    }
                    return false;
                });

                $('.map-drop').bind(FUI.env.ua.clickEventDown, { handle: this }, function (event) {
                    var droptop = parseInt($('.map-drop').css('top'));
                    var mouse = { x: 0, y: 0 };
                    var e = event || window.event;
                    if (navigator.msMaxTouchPoints) {
                        e = event.originalEvent;
                    ...
                    mouse.y = e.clientY;
                    //
                    $('.map-area').bind(FUI.env.ua.clickEventMove, { handle: event.data.handle }, function (event) {
                        var e = event || window.event;
                        if (FUI.env.ua.ontouch) {
                            event.preventDefault();
                            e = event.originalEvent.touches[0] || event.originalEvent.changedTouches[0];
                        ...
                        //
                        var top = parseInt($('.map-drop').css('top')) + (e.clientY - mouse.y);
                        mouse.x = e.clientX;
                        mouse.y = e.clientY;
                        ...
                        //
                        var step = 90 / (event.data.handle.layerSplit - event.data.handle.defaultLayer - 1) / 5;
                        var level = (droptop - top) / step;
                        $('.map-drop').css({ "top": top });
                        //
                        if (level != 0) {
                            _zoom.call(event.data.handle, level);
                            droptop = top;
                        }
                    });
                });

                $(document).bind(FUI.env.ua.clickEventUp, function (event) {
                    $('.map-area').unbind(FUI.env.ua.clickEventMove);
                });
            }
        });
